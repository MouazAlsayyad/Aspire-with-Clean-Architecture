...
public class SendMembershipEmailUseCase : BaseUseCase
{
    ...
    public SendMembershipEmailUseCase(
        ...
        CancellationToken cancellationToken = default)
    {
        return await ExecuteAndSaveAsync(async ct =>
        {
            try
            {
                ...
                var subject = "Membership Subscription";

                _emailManager.ValidateEmailRequest(dto.Email, subject, htmlContent);

                var priority = _emailManager.GetPriorityForEmailType(EmailType.Membership);

                // Prepare BCC if tenant email is provided
                var bccList = new List<string>();
                if (!string.IsNullOrEmpty(dto.TenantEmail))
                {
                    bccList.Add(dto.TenantEmail);
                }
            ...
            catch (Exception ex)
            {
                return Result.Failure<EmailLogDto>(
                    DomainErrors.General.InternalError($"Failed to send membership email: {ex.Message}"));
            }
        }, cancellationToken);
    }
}
