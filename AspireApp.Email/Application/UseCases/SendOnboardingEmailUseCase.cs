...
public class SendOnboardingEmailUseCase : BaseUseCase
{
    ...
    public SendOnboardingEmailUseCase(
        ...
        CancellationToken cancellationToken = default)
    {
        return await ExecuteAndSaveAsync(async ct =>
        {
            try
            {
                var htmlContent = _templateProvider.GetOnboardingTemplate(dto.OnboardingUrl);
                var subject = "Stripe Onboarding Step";

                _emailManager.ValidateEmailRequest(dto.Email, subject, htmlContent);

                var priority = _emailManager.GetPriorityForEmailType(EmailType.Onboarding);
            ...
            catch (Exception ex)
            {
                return Result.Failure<EmailLogDto>(
                    DomainErrors.General.InternalError($"Failed to send onboarding email: {ex.Message}"));
            }
        }, cancellationToken);
    }
}
