...
public class SendPayoutOTPUseCase : BaseUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await ExecuteAndSaveAsync(async ct =>
        {
            try
            {
                var htmlContent = _templateProvider.GetPayoutOtpTemplate(dto.ClubName, dto.Otp);
                var subject = "Payout OTP Confirmation";

                _emailManager.ValidateEmailRequest(dto.Email, subject, htmlContent);

                var priority = _emailManager.GetPriorityForEmailType(EmailType.PayoutOTP);
            ...
            catch (Exception ex)
            {
                return Result.Failure<EmailLogDto>(
                    DomainErrors.General.InternalError($"Failed to send payout OTP email: {ex.Message}"));
            }
        }, cancellationToken);
    }
}
