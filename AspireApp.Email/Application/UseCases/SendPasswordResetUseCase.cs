...
public class SendPasswordResetUseCase : BaseUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await ExecuteAndSaveAsync(async ct =>
        {
            try
            {
                var htmlContent = _templateProvider.GetPasswordResetTemplate(dto.ResetUrl);
                var subject = "Password Reset Request";

                _emailManager.ValidateEmailRequest(dto.Email, subject, htmlContent);

                var priority = _emailManager.GetPriorityForEmailType(EmailType.ForgotPassword);
            ...
            catch (Exception ex)
            {
                return Result.Failure<EmailLogDto>(
                    DomainErrors.General.InternalError($"Failed to send password reset email: {ex.Message}"));
            }
        }, cancellationToken);
    }
}
